...
class Neo4jConnector:
    ...
    def create_knowledge_graph(self, triplets):
        """Create knowledge graph in Neo4j from triplets."""
        with self.driver.session() as session:
            ...
            for entity in unique_entities:
                ...
                SET n.caption = $name
                """
                session.run(cypher_query, name=entity)
            
            # Then create relationships
            for triplet in triplets:
                # Convert relation and labels to valid Neo4j identifiers
                rel_type = triplet['relation'].upper().replace(' ', '_').replace('-', '_')
                subj_label = triplet['subject'].replace(' ', '_').replace('-', '_').upper()
                obj_label = triplet['object'].replace(' ', '_').replace('-', '_').upper()
                
                # Add 'E_' prefix if label doesn't start with letter
                if not subj_label[0].isalpha():
                    subj_label = 'E_' + subj_label
                if not obj_label[0].isalpha():
                    obj_label = 'E_' + obj_label
